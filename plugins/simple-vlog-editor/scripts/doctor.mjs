...
const discovery = process.env.SVE_RUNTIME_MODE === 'dev' ? { found: null, searched: [] } : discoverEditor();
if (process.env.SVE_RUNTIME_MODE === 'dev') {
  ...
  fail('SimpleVlogEditor installed', notInstalledMessage(discovery),
    ['Looked only in the installer\'s default folders:', ...searchedPlaces().map((place) => `  - ${place}`)].join('\n'));
}

// --------------------------------------------------------------- MCP session

async function session() {
  const environment = { ...process.env, SVE_SKIP_UPDATE_CHECK: '1' };
  const probe = startProbe({ args: [path.join(PLUGIN_ROOT, 'scripts', 'mcp-launcher.mjs')], cwd: PLUGIN_ROOT, env: environment });
...
if (launchable && target && !noLaunch) await session();
